...
void JUNE_plots_h60p_test_noCOS(){
  ...
  //  TCanvas* cnvs1= new TCanvas("cnvs1", " East Cryostat MC +60",1,1,600,500);
  TH1F *h1 = new TH1F("h1"," TPC3 East Cryostat MC +60 (COL)", 100., -1.0, 1.0);
  h1->GetXaxis()->SetTitle("  angle wrt horizontal");

  //  TCanvas* cnvs2= new TCanvas("cnvs2", " East Cryostat MC +60",1,1,600,500);
  TH1F *h2 = new TH1F("h2"," TPC3 East Cryostat MC +60 (IN1)", 100.,-1.0, 1.0); //-1.0, 1.0);0., 7.0);
  h2->GetXaxis()->SetTitle("  angle wrt horizontal");

  //TCanvas* cnvs3= new TCanvas("cnvs3", " East Cryostat MC +60",1,1,600,500);
  TH1F *h3 = new TH1F("h3"," TPC3 East Cryostat MC +60 (IN2)", 100., -1.0, 1.0); //0., 7.0);
  h3->GetXaxis()->SetTitle("  angle wrt horizontal");

  //TCanvas* cnvs7= new TCanvas("cnvs7", " West Cryostat MC +60",1,1,600,500);
  TH1F *h4 = new TH1F("h4"," TPC3 West Cryostat MC +60 (COL)", 100., -1.0, 1.0); //0., 7.0);
  h4->GetXaxis()->SetTitle("  angle wrt horizontal");

  //TCanvas* cnvs8= new TCanvas("cnvs8", " West Cryostat MC +60",1,1,600,500);
  TH1F *h5 = new TH1F("h5"," TPC3 West Cryostat MC +60 (IN1)", 100., -1.0, 1.0); //0., 7.0);
  h5->GetXaxis()->SetTitle("  angle wrt horizontal");

  //TCanvas* cnvs9= new TCanvas("cnvs9", " West Cryostat MC +60",1,1,600,500);
  TH1F *h6 = new TH1F("h6"," TPC3 West Cryostat MC +60 (IN2)", 100., -1.0, 1.0); //0., 7.0);
  h6->GetXaxis()->SetTitle("  angle wrt horizontal");
  ...
  for(int aa2 = 0; aa2<aa; aa2++){
    ...
    h1->Fill(  (TMath::Pi()/3.0) + z_SEL12_East_MC_col[aa2]  );
  ...
  for(int bb2 = 0; bb2<bb; bb2++){
    ...
    h2->Fill( (TMath::Pi()/3.0) + z_SEL12_East_MC_in1[bb2] );
    }


  for(int cc2 = 0; cc2<cc; cc2++){
    z_SEL12_East_MC_in2[cc2] =  abs( (TMath::Pi()/2.0) - SEL12_East_MC_in2[cc2] );
    h3->Fill(  (TMath::Pi()/3.0) + z_SEL12_East_MC_in2[cc2] );
    }
  ...
  for(int dd2 = 0; dd2<dd; dd2++){
    ...
    h4->Fill( (TMath::Pi()/3.0) + z_SEL12_West_MC_col[dd2]  );
     }
  ...
  for(int ee2 = 0; ee2<ee; ee2++){
     ...
     h5->Fill( (TMath::Pi()/3.0) + z_SEL12_West_MC_in1[ee2] );
     }


  for(int ff2 = 0; ff2<ff; ff2++){
    z_SEL12_West_MC_in2[ff2] =  abs( (TMath::Pi()/2.0) - SEL12_West_MC_in2[ff2] );
    h6->Fill(  (TMath::Pi()/3.0) + z_SEL12_West_MC_in2[ff2] );
  }
  ...
  h6->Draw("HIST");
   ...
   cnvs1->SaveAs("MC_tpc3_sel12_East_file_HAngle_p60_test_col_noCOS.pdf");

  cnvs2->SaveAs("MC_tpc3_sel12_East_file_HAngle_p60_test_in1_noCOS.pdf");

  cnvs3->SaveAs("MC_tpc3_sel12_East_file_HAngle_p60_test_in2_noCOS.pdf");


  cnvs4->SaveAs("MC_tpc3_sel12_West_file_HAngle_p60_test_col_noCOS.pdf");

  cnvs5->SaveAs("MC_tpc3_sel12_West_file_HAngle_p60_test_in1_noCOS.pdf");

  cnvs6->SaveAs("MC_tpc3_sel12_West_file_HAngle_p60_test_in2_noCOS.pdf");
