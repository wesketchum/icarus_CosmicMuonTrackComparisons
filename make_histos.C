...
void make_histos(bool verbose=false) {
  ...
  TH1F *h_azangle_mc   = new TH1F("h_azangle_mc","Relative Frequency vs  Azimuthal Angle: East Cryostat", 100., 0., 1.6);
  TH1F *h_azangle_data = new TH1F("h_azangle_data","Relative Frequency vs Azimuthal Angle: East Cryostat", 100., 0., 1.6);
  h_azangle_mc->GetXaxis()->SetTitle(" Azimuthal Angle in radians ");
  h_azangle_mc->GetYaxis()->SetTitle("Relative Frequency");
  ...
  TTreeReaderArray<bool> ontraj2_i1(myReader2, "trk.hits0.ontraj");
   ...
   while (myReader.Next()) {
     ...
     h_azangle_mc->Fill( TMath::Atan(x_c[last_i]/z_c[last_i]) );
     if(verbose) 
   ...
   while (myReader2.Next()) {
     ...
     h_azangle_data->Fill( TMath::Atan(x_c[last_i]/z_c[last_i]) );
     if(verbose) 
       cout << "MC angle " << TMath::Atan(x_c[last_i]/z_c[last_i]) << endl;
   ...
   myOutput_file.Write(h_azangle_mc);
   myOutput_file.Write(h_azangle_data);
   myOutput_file.Close();


   //normalizing MC distribution
   TH1*h1 = (TH1*)(h_azangle_mc->Clone("h1"));
   h1->Scale(1./h1->Integral());

   //normalizing data distribution
   TH1*h2 = (TH2*)(h_azangle_data->Clone("h2"));
