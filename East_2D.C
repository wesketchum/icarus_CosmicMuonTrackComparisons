...
void East_2D(bool verbose=false) {
  ...
  ifstream ifile1("OUTFILE_mc_my_outputNUMI_tpc2_sel12_East_file_Length_Az_col.txt");
  ifstream ifile2("OUTFILE_mc_my_outputNUMI_tpc2_sel12_East_file_Cint_Az_col.txt");
  ifstream ifile3("OUTFILE_data_my_outputNUMI_tpc2_sel12_East_file_Length_Az_col.txt");
  ifstream ifile4("OUTFILE_data_my_outputNUMI_tpc2_sel12_East_file_Cint_Az_col.txt");

  ifstream ifile5("OUTFILE_mc_my_outputNUMI_tpc3_sel12_East_file_Length_Az_col.txt");
  ifstream ifile6("OUTFILE_mc_my_outputNUMI_tpc3_sel12_East_file_Cint_Az_col.txt");
  ifstream ifile7("OUTFILE_data_my_outputNUMI_tpc3_sel12_East_file_Length_Az_col.txt");
  ifstream ifile8("OUTFILE_data_my_outputNUMI_tpc3_sel12_East_file_Cint_Az_col.txt");
  ...
  while(ifile5>>MCA1>>MCL1)
    {
       Az_MC1[MC_L1] =  MCA1;
      Length_MC1[MC_L1] = MCL1;

      //cout<<Az_MC1[MC_L1]<<" "<<Length_MC1[MC_L1]<<endl;
      MC_L1++;
    }

  while(ifile6>>MCAz1>>MCC1)
    {
      
      Cint_MC1[MC_C1] = MCC1;
      //cout<<Cint_MC1[MC_C1]<<endl;
      MC_C1++;
    }

  while(ifile7>>DATAA1>>DATAL1)
    {
       Az_DATA1[DATA_L1] = DATAA1;
      Length_DATA1[DATA_L1] = DATAL1;
      //cout<<Az_DATA1[DATA_L1]<<" "<<Length_DATA1[DATA_L1]<<endl;

      DATA_L1++;
    }

  while(ifile8>>DATAAz1>>DATAC1)
    {
      Cint_DATA1[DATA_C1] = DATAC1;

      //cout<<Cint_DATA1[DATA_C1]<<endl;
      DATA_C1++;
    }
  ...
  TH2F *h_sx2Dc_mc   = new TH2F("h_sx2Dc_mc","Azimuthal Angle vs  Start (X): East Cryostat", 100., 0., 1.6, 100., -100., -20.);
  TH2F *h_sx2Dc_data = new TH2F("h_sx2Dc_data","Azimuthal Angle vs Start (X): East Cryostat", 100., 0., 1.6, 100., -100., -20.);
  h_sx2Dc_mc->GetYaxis()->SetTitle("Start (X) in cm ");
  h_sx2Dc_mc->GetXaxis()->SetTitle("Azimuthal angle in radians");


  TH2F *h_sx2D1_mc   = new TH2F("h_sx2D1_mc","Azimuthal Angle vs  Start (X): East Cryostat", 100., 0., 1.6, 100., -100., -20.);
  TH2F *h_sx2D1_data = new TH2F("h_sx2D1_data","Azimuthal Angle vs Start (X): East Cryostat", 100., 0., 1.6, 100., -100., -20.);
  h_sx2D1_mc->GetYaxis()->SetTitle("Start (X) in cm ");
  h_sx2D1_mc->GetXaxis()->SetTitle("Azimuthal angle in radians");

  TH2F *h_sx2D2_mc   = new TH2F("h_sx2D2_mc","Azimuthal Angle vs  Start (X): East Cryostat", 50., 0., 1.6, 100., -100., -20.);
  TH2F *h_sx2D2_data = new TH2F("h_sx2D2_data","Azimuthal Angle vs Start (X): East Cryostat", 50., 0., 1.6, 100., -100., -20.);
...
for(int n3 = 0; n3 < MC_L1; n3++){

  h_l2D1_mc->Fill(Az_MC1[n3] , Length_MC1[n3]);
  h_ci12D_mc->Fill(Az_MC1[n3] , Cint_MC1[n3]);

 }


for(int n4 = 0; n4 < DATA_L1; n4++){
  h_l2D1_data->Fill(Az_DATA1[n4]  , Length_DATA1[n4]);
  h_ci12D_data->Fill(Az_DATA1[n4] , Cint_DATA1[n4]);
...
TH2*h1l2D1 = (TH2*)(h_l2D1_mc->Clone("h1l2D1"));
...
h2BB2D->Scale(1./h2BB2D->Integral());
   ...
   h1l2Dc_prof->SetLineColor(kRed-5);
   h2l2Dc_prof->SetLineColor(kTeal);
   ...
   legendl2Dc->AddEntry(h1l2Dc_prof,"MC tpc 2");
   legendl2Dc->AddEntry(h2l2Dc_prof,"Data tpc 2");
   legendl2Dc->AddEntry(h1l2D1_prof,"MC tpc 3");
   legendl2Dc->AddEntry(h2l2D1_prof,"Data tpc 3");

   legendl2Dc->Draw();
   cnvs_l2Dc->Update();

   TFile output_file1("my_output_file_2D_my_outputNUMI_SEL12_East23_fileLength_Az_col.root","RECREATE");
   h1l2Dc_prof->Write();
   h2l2Dc_prof->Write();
   cnvs_l2Dc->Write();

   cnvs_l2Dc->SaveAs("2D_my_outputNUMI_SEL12_East23_fileLength_Az_col.pdf");
   ...
   h1ci22D_prof->SetLineColor(kRed-5);
   h2ci22D_prof->SetLineColor(kTeal);
   ...
   legendAA2D->AddEntry(h1AA2D,"MC tpc 2");
   legendAA2D->AddEntry(h2AA2D,"Data tpc 2");
   legendAA2D->AddEntry(h1BB2D,"MC tpc 3");
   legendAA2D->AddEntry(h2BB2D,"Data tpc 3");

   legendAA2D->Draw();
   cnvs_ci22D->Update();


   TFile output_file22("my_output_file_2D_my_outputNUMI_SEL12_East23_fileCint_Az_col.root","RECREATE");
   h1ci22D_prof->Write();
   h2ci22D_prof->Write();
   cnvs_ci22D->Write();


   cnvs_ci22D->SaveAs("2D_my_outputNUMI_SEL12_East23_fileInt_az_col.pdf");
