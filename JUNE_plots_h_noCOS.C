...
void JUNE_plots_h_noCOS(){
  ...
  TH1F *h1 = new TH1F("h1","TPC3 East Cryostat MC (COL)", 100., 0.0, 2.0);
  h1->GetXaxis()->SetTitle("  angle wrt horizontal in radians");

  //  TCanvas* cnvs2= new TCanvas("cnvs2", "TPC3 East Cryostat MC",1,1,600,500);
  TH1F *h2 = new TH1F("h2","TPC3 East Cryostat MC (IN1)", 50.,0.0, 2.0); //0.0, 2.0);0., 7.0);
  h2->GetXaxis()->SetTitle("  angle wrt horizontal in radians");

  //TCanvas* cnvs3= new TCanvas("cnvs3", "TPC3 East Cryostat MC",1,1,600,500);
  TH1F *h3 = new TH1F("h3","TPC3 East Cryostat MC (IN2)", 100., 0.0, 2.0); //0., 7.0);
  h3->GetXaxis()->SetTitle("  angle wrt horizontal in radians");

  //TCanvas* cnvs7= new TCanvas("cnvs7", "TPC3 West Cryostat MC",1,1,600,500);
  TH1F *h4 = new TH1F("h4","TPC3 West Cryostat MC (COL)", 100., 0.0, 2.0); //0., 7.0);
  h4->GetXaxis()->SetTitle("  angle wrt horizontal in radians");

  //TCanvas* cnvs8= new TCanvas("cnvs8", "TPC3 West Cryostat MC",1,1,600,500);
  TH1F *h5 = new TH1F("h5","TPC3 West Cryostat MC (IN1)", 100., 0.0, 2.0); //0., 7.0);
  h5->GetXaxis()->SetTitle("  angle wrt horizontal in radians");

  //TCanvas* cnvs9= new TCanvas("cnvs9", "TPC3 West Cryostat MC",1,1,600,500);
  TH1F *h6 = new TH1F("h6","TPC3 West Cryostat MC (IN2)", 100., 0.0, 2.0); //0., 7.0);
  h6->GetXaxis()->SetTitle("  angle wrt horizontal in radians");
  ...
  for(int aa2 = 0; aa2<aa; aa2++){
    ...
    h1->Fill(z_SEL12_East_MC_col[aa2]);
  ...
  for(int dd2 = 0; dd2<dd; dd2++){
    ...
    a<<SEL12_West_MC_col[dd2]<<" "<<z_SEL12_West_MC_col[dd2]<<endl;
  ...
  for(int ee2 = 0; ee2<ee; ee2++){
     ...
     a<<SEL12_West_MC_in1[ee2]<<" "<<z_SEL12_West_MC_in1[ee2]<<endl;

    h5->Fill(z_SEL12_West_MC_in1[ee2]);
     }
  ...
  h6->Draw("HIST");
   ...
   cnvs1->SaveAs("MC_tpc3_sel12_East_file_HAngle_col_noCOS.pdf");

  cnvs2->SaveAs("MC_tpc3_sel12_East_file_HAngle_in1_noCOS.pdf");

  cnvs3->SaveAs("MC_tpc3_sel12_East_file_HAngle_in2_noCOS.pdf");


  cnvs4->SaveAs("MC_tpc3_sel12_West_file_HAngle_col_noCOS.pdf");

  cnvs5->SaveAs("MC_tpc3_sel12_West_file_HAngle_in1_noCOS.pdf");

  cnvs6->SaveAs("MC_tpc3_sel12_West_file_HAngle_in2_noCOS.pdf");
